        ...
        <form onSubmit={handleSearch} className="mb-6">
          ...
          <div className={`gap-3 grid grid-cols-2 md:flex ${showFilters ? 'flex' : 'hidden md:flex'}`}>
            <input
              ...
              onChange={handleFilterChange}
              suppressHydrationWarning
              className={`w-full md:w-36 ${inputCls}`}
            />
            <input
              name="area"
              placeholder="Area"
            ...
            {services.map((service) => (
              ...
              >
                <div className="flex justify-between items-start mb-4">
                  <span className="inline-block px-2 py-1 bg-gray-50 dark:bg-neutral-800 text-gray-600 dark:text-gray-400 text-xs font-medium rounded uppercase tracking-wider">
                    {service.category}
                  </span>
